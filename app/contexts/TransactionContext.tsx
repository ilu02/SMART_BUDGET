...
export function TransactionProvider({ children }: { children: ReactNode }) {
    ...
    const addTransaction = async (transactionData: Omit<Transaction, 'id' | 'userId' | 'createdAt' | 'updatedAt'>): Promise<boolean> => {
        ...
        // If the browser returns a datetime like "2025-10-14T10:00"
        if (correctedDateString.includes('T') && !correctedDateString.endsWith('Z')) {
            ...
            // For date-only inputs, use the current local time instead of midnight
            const now = new Date();
        ...
        try {
            const response = await fetch('/api/transactions', {
                ...
                body: JSON.stringify({
                    ...transactionData,
                    date: correctedDateString, // Use the corrected UTC date string
                    userId: user.id,
                }),
            });
